...
function apiCard(image) {
    return `
        <div class="card">
            <img src="${image.image_url}"alt="${image.title}"class="card-image">
            <div class="card-content">
                <h2 class="card-title">${image.title}</h2>
                <p class="card-category"> Category: ${image.category}</p>
            </div>
        </div>
    `;
}
...
async function dataCard() {
    ...
    try {
        ...
        galleryData.innerHTML = images.map(image => apiCard(image)).join('');
    } catch (error) {
...
document.addEventListener('DOMContentLoaded', dataCard);
